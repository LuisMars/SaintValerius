...
public class Shake {
    ...
    public void update(float dt, Camera camera, Vector2 center){
        ...
        if (shakeDuration > 0){
            ...
            camera.position.x = ((int) (center.x + deltaX * amplitude * (falloff ? Math.min(shakeDuration, 1f) : 1f)));
            camera.position.y = ((int) (center.y + deltaY * amplitude * (falloff ? Math.min(shakeDuration, 1f) : 1f)));
            camera.update();
        }
    }
     ...
     */
    public static void shake(float d){
        shakeDuration = d;
    }
}